...
if __name__ == "__main__":
    ...
    for millesime, version_url in current_versions_urls:
        ...
        departments = get_all_links(f"{version_url}geojson/communes/")
        ...
        for department in departments:
            ...
            communes = get_all_links(f"{version_url}geojson/communes/{department}/")
            communes = [c for c in communes if c != ".."]
            if not communes:
                logger.error(f"Aucune commune trouvée pour le département {department} de la version {version_url}")
                continue
            # sauvegarder la liste des pour créer l'historique
            # il faut vérifier que le dossier fichier existe sinon le créer en initialisant le fichier avec une structure
            for commune in communes:
                logger.info(f"===Traitement de la commune {commune}")
                # === Récupérer le fichier, ajouter le millesime s'il n'existe pas / créer le fichier s'il n'existe pas
                path = f"cadastre/suivi_fichiers/{millesime}/{department}/"
                path_dataset = f"cadastre/data/{millesime}/{department}/"
                os.makedirs(path, exist_ok=True)
                commune_history_file_path = os.path.join(path, f"{commune}.json")
                default_json_structure = {"files": {}, "last_checked": datetime.now().isoformat()}
                if not os.path.exists(commune_history_file_path):
                    with open(commune_history_file_path, 'w') as f:
                        json.dump({millesime: default_json_structure}, f)
                with open(commune_history_file_path, 'r') as f:
                    commune_json_history = json.load(f)
                if millesime not in commune_json_history:
                    commune_json_history[millesime] = default_json_structure

                # créer le dossier commune dans cadastra/data
                commune_path = f"{path_dataset}/{commune}/"
                os.makedirs(f"{commune_path}", exist_ok=True)

                # récupérer les fichiers de la commune
                commune_files_online = get_file_name_date_size(f"{version_url}geojson/communes/{department}/{commune}/")
                # afficher les fichiers trouvés
                print(commune_files_online)

                # ne pas inclure le raw folder
                commune_files_online = [f for f in commune_files_online if f != "raw"]
                commune_files_online = [f for f in commune_files_online if f != ".."]
                if not commune_files_online:
                    logger.error(f"Aucun fichier trouvé pour la commune {commune} du département {department} de la version {millesime}")
                    anomaly_logger.error(f"Aucun fichier trouvé pour la commune {commune} du département {department} de la version {millesime}")
                    continue
                
                # pour chaque fichier, ajouter l'historique, avec la date, et la taille
                # get_folder_name_and_date devrait être bon car la structure est la même, sauf que l'on a la taille à la fin
                # faire un test et print le premier

                # liste des fichiers possible 'cadastre-01001-batiments.json.gz, cadastre-01001-communes.json.gz, feuilles, lieux_dits, parcelles, prefixes_sections, sections, subdivisions_fiscales'
                # la structure est donc cadastre-<code_insee>-<type>.json.gz ou cadastre-<code_insee>-<type>.json
                # available_files = ['batiments', 'communes', 'feuilles', 'lieux_dits', 'parcelles', 'prefixes_sections', 'sections', 'subdivisions_fiscales']
                # ne pas se  limiter à ces fichiers, il peut y avoir d'autres fichiers
                for file_name, date, size in commune_files_online:
                    logger.info(f"Traitement du fichier {file_name}")
                    if file_name not in commune_json_history[millesime]["files"]: 
                        # Log -> le fichier n'existe pas, on l'intialise
                        commune_json_history[millesime]["files"][file_name] = {"current": {"last_modified": date, "size": size}, "history": []}
                    
                    # Récupérer le dernier historique
                    last_history = commune_json_history[millesime]["files"][file_name]["history"][-1] if commune_json_history[millesime]["files"][file_name]["history"] else []

                    # Vérifier si la date et la taille sont les mêmes
                    if last_history and last_history.get("last_modified") == date and last_history.get("size") == size:
                        # Log -> le fichier n'a pas changé
                        logger.info(f"Le fichier {file_name} n'a pas changé pour la commune {commune} du département {department} de la version {millesime}")
                        # pas besoin de télécharger le nouveau fichier mais vérifier s'il existe dans le dossier
                        if os.path.exists(os.path.join(commune_path, file_name)):
                            continue
                        else:
                            anomaly_logger.warning(f"Le fichier {file_name} devrait exister, téléchargement")
                    else:
                        # Ajouter une alerte si la taille du fichier a changé dans cadastre/logs/anomalies.log
                        if last_history and last_history.get("size") != size:
                            anomaly_logger.warning(f"Le fichier {file_name} a changé de taille pour la commune {commune} du département {department} de la version {millesime} -> {last_history.get('size')} -> {size}")
                        if last_history and last_history.get("last_modified") != date:
                            anomaly_logger.warning(f"Le fichier {file_name} a changé de date de modification pour la commune {commune} du département {department} de la version {millesime} -> {last_history.get('last_modified')} -> {date}")
                    
                    # Ajouter l'historique, si c'est la premier fois, ajout le last_modified et la taille sont les mêmes
                    commune_json_history[millesime]["files"][file_name]["history"].append({"last_modified": date, "size": size, "checked_on": datetime.now().isoformat()})
                    # mettre à jour le current
                    commune_json_history[millesime]["files"][file_name]["current"] = {"last_modified": date, "size": size}
                    
                    # Télchargement du fichier, s'il existait déjà un autre fichier, on va renommer l'ancien avec sa date de modification
                    file_path = os.path.join(commune_path, file_name)
                    if os.path.exists(file_path):
                        os.rename(file_path, f"{file_path}_{last_history.get('last_modified')}")
                    # Télécharger le fichier
                    download_url = f"{version_url}geojson/communes/{department}/{commune}/{file_name}"
                    result = download_file(download_url, file_path)
                    if not result:
                        # Log -> le fichier n'a pas pu être téléchargé ajouter une alerte dans cadastre/logs/anomalies.log
                        anomaly_logger.error(f"Le fichier {file_name} n'a pas pu être téléchargé pour la commune {commune} du département {department} de la version {millesime}")
                        continue
                    # Log -> le fichier a été téléchargé
                    logger.info(f"Le fichier {file_name} a été téléchargé pour la commune {commune} du département {department} de la version {millesime}")

                # sauvegarder le fichier de l'historique
                with open(commune_history_file_path, 'w') as f:
                    json.dump(commune_json_history, f)
                logger.info(f"L'historique du fichier {commune_history_file_path} a été mis à jour pour la commune {commune} du département {department} de la version {millesime}")
    # Log des requêtes
