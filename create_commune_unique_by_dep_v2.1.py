...
def process_communes_by_dep(conn, departement_table_name, millesime, previous_millesime):
    ...
    en gérant les versions avec une clé composite (code_commune, millesime).
    """
    ...
    """
    current_communes = execute_query_safe(
        conn,
        current_query,
        error_message=f"Erreur récupération communes actuelles pour {code_departement}"
    )

    if not current_communes:
        logging.warning(f"Aucune donnée actuelle trouvée pour {departement_table_name}, millésime {millesime}.")
        return
    ...
    if previous_millesime:
        previous_query = f"""
            ...
            FROM reference.communes_unique
            WHERE code_departement = '{code_departement}' AND millesime = '{previous_millesime}';
        """
        previous_communes = {
            row[0]: {"nom_commune": row[1], "updated": row[2], "geom": row[3]}
            for row in execute_query_safe(conn, previous_query)
        }

    # Étape 3 : Comparaison et gestion des communes actuelles
    for commune in current_communes:
        code_commune, nom_commune, updated, geom = commune

        # Vérifier si la commune existait dans le millésime précédent
        if code_commune in previous_communes:
            pre_values = previous_communes[code_commune]

            # Comparer et enregistrer les changements
            compare_and_update(
                conn, "reference.communes_unique", code_commune, "nom_commune",
                pre_values["nom_commune"], nom_commune, millesime
            )
            compare_and_update(
                conn, "reference.communes_unique", code_commune, "updated",
                pre_values["updated"], updated, millesime
            )

            # Comparer et valider les géométries
            current_geom_query = f"SELECT ST_MakeValid(geom) FROM cadastre_{millesime}.{departement_table_name} WHERE id = '{code_commune}'"
            previous_geom_query = f"SELECT geom FROM reference.communes_unique WHERE code_commune = '{code_commune}' AND millesime = '{previous_millesime}'"
            ...
            # Si la commune est nouvelle, l'insérer avec le millésime actuel
            insert_query = f"""
                INSERT INTO reference.communes_unique (code_commune, millesime, code_departement, nom_commune, created, updated, geom)
                VALUES ('{code_commune}', '{millesime}', '{code_departement}', '{nom_commune}', NOW(), '{updated}', '{geom}');
            """
            execute_query_safe(conn, insert_query, error_message=f"Erreur insertion nouvelle commune {code_commune}")

    # Étape 4 : Gestion des communes manquantes (absentes dans le millésime actuel)
    for code_commune, pre_values in previous_communes.items():
        if code_commune not in {row[0] for row in current_communes}:
            current_geom_query = f"SELECT geom FROM reference.communes_unique WHERE code_commune = '{code_commune}' AND millesime = '{previous_millesime}'"
            handle_missing_commune(conn, code_commune, code_departement, millesime, current_geom_query)

    # Étape 5 : Enregistrement des statistiques pour le millésime
    stats_query = f"""
        INSERT INTO stats.stats_communes (millesime, code_departement, nb_communes)
        SELECT '{millesime}', '{code_departement}', COUNT(*)
        FROM cadastre_{millesime}.{departement_table_name};
    """
    execute_query_safe(conn, stats_query, error_message=f"Erreur enregistrement statistiques pour {code_departement}, millésime {millesime}")

    logging.info(f"Traitement terminé pour {departement_table_name}, millésime {millesime}.")
